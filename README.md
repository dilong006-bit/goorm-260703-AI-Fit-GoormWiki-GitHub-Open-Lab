...
Token 없이도 동작하지만, Rate Limit(시간당 60회)에 걸릴 수 있습니다. Token 설정 시 시간당 5,000회로 완화됩니다.
...
├── hooks/            useRepositories, useSearch, useDebounce
...
- **수정:** `.prose-github`에 다크 그레이(`#1f2328`) 텍스트 및 Typography CSS 변수 오버라이드 적용

## Vercel 배포

1. 이 저장소를 GitHub에 push
2. [vercel.com](https://vercel.com) → **Add New → Project** → 저장소 Import (Framework Preset: **Vite** 자동 감지)
3. **Settings → Environment Variables**에 `VITE_GITHUB_USERNAME`, `VITE_GITHUB_TOKEN` 추가 (Production/Preview/Development)
4. **Deploy** — `vercel.json`의 SPA fallback rewrite로 React Router 경로가 정상 동작

## 라이선스

Private — AI-Fit GoormWiki Open Lab Phase 1 MVP
